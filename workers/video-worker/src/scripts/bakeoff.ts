 ...
 *
 *   npx tsx src/scripts/bakeoff.ts [--limit N] [--models a,b,c]
 */
...
import { tagImageWith } from '../ai/tag.js';
...
async function main() {
  ...
  for (const r of rows) {
    ...
    images.set(r.id, Buffer.from(await dl.data.arrayBuffer()).toString('base64'));
  }
  console.log(`downloaded ${images.size} proxies\n`);
  ...
  for (const m of models) {
    ...
    for (const r of rows) {
      ...
      if (!b64) continue;
      try {
        const t = await tagImageWith(m, { systemPrompt: prompt, imageBase64: b64 });
        micro += t.microCents;
        ms += t.latencyMs;
        ok++;
